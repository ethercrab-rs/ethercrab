...
async fn main() -> Result<(), Error> {
    ...
    let group = SlaveGroup::<MAX_SLAVES, PDI_LEN>::new(|slave| {
        Box::pin(async {
            ...
            let profile = match slave.read_sdo::<u32>(0x1000, 0).await {
            ...
            if slave.name() == "AKD" {
                ...
                slave.write_sdo(0x1c12, 1, 0x1702u16).await?;
                slave.write_sdo(0x1c12, 0, 0x01u8).await?;

                // Must set both read AND write SDOs for AKD otherwise it times out going into OP
                slave.write_sdo(0x1c13, 0, 0u8).await?;
                slave.write_sdo(0x1c13, 1, 0x1B01u16).await?;
                slave.write_sdo(0x1c13, 0, 0x01u8).await?;
...

                {
                    ...
                    let encoder_increments = slave.read_sdo::<u32>(0x608f, 1).await?;
                    let num_revs = slave.read_sdo::<u32>(0x608f, 2).await?;

                    let gear_ratio_motor = slave.read_sdo::<u32>(0x6091, 1).await?;
                    let gear_ratio_final = slave.read_sdo::<u32>(0x6091, 2).await?;

                    let feed = slave.read_sdo::<u32>(0x6092, 1).await?;
                    let shaft_revolutions = slave.read_sdo::<u32>(0x6092, 2).await?;
