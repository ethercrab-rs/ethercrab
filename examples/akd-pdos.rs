...
async fn main() -> Result<(), Error> {
    ...
    for slave in group.iter(&client) {
        ...
        if slave.name() == "AKD" {
            // Manual p. 48:
            // A u32 written to a PDO is (16 bit addr, 8 bit sub-addr, 8 bit length)
            ...
            // slave.sdo_write(0x1600, 0, 0u8).await.expect("0x1600 0u8");
            // slave.sdo_write(0x1600, 1, 0x6040_0010u32).await.expect("0x1600 0x6040_0010u32");
            // slave.sdo_write(0x1600, 2, 0x607a_0020u32).await.expect("0x1600 0x607a_0020u32");
            // slave.sdo_write(0x1600, 0, 2u8).await.expect("0x1600 2u8");

            // slave.sdo_write(0x1a00, 0, 0u8).await.expect("0x1a00 0u8");
            // slave.sdo_write(0x1a00, 1, 0x6041_0010u32).await.expect("0x1a00 0x6041_0010u32");
            // slave.sdo_write(0x1a00, 2, 0x6064_0020u32).await.expect("0x1a00 0x6064_0020u32");
            // slave.sdo_write(0x1a00, 0, 2u8).await.expect("0x1a00 2u8");

            // slave.sdo_write(0x1c12, 0, 0u8).await.expect("0x1c12, 0, 0u8");
            // slave.sdo_write(0x1c12, 1, 0x1600u16).await.expect("0x1c12, 1, 0x1600u16");
            // slave.sdo_write(0x1c12, 0, 0x01u8).await.expect("0x1c12, 0, 0x01u8");

            // slave.sdo_write(0x1c13, 0, 0u8).await.expect("0x1c13, 0, 0u8");
            // slave.sdo_write(0x1c13, 1, 0x1a01u16).await.expect("0x1c13, 1, 0x1a01u16");
            // slave.sdo_write(0x1c13, 0, 0x01u8).await.expect("0x1c13, 0, 0x01u8");

            // // Opmode - Cyclic Synchronous Position
            // slave.sdo_write(0x6060, 0, 0x08u8).await.expect("CSP");

            // --- CSV ---

            slave.sdo_write(0x1600, 0, 0u8).await.expect("0x1600 0u8");
            slave
                .sdo_write(0x1600, 1, 0x60ff_0020u32)
                .await
                .expect("0x1600 0x60ff_0020u32");
            slave
                .sdo_write(0x1600, 2, 0x6040_0010u32)
                .await
                .expect("0x1600 0x6040_0010u32");
            // slave.sdo_write(0x1600, 3, 0x6060_0010u32).await.expect("0x1600 0x6060_0010u32");
            slave.sdo_write(0x1600, 0, 2u8).await.expect("0x1600 2u8");

            slave.sdo_write(0x1a00, 0, 0u8).await.expect("0x1a00 0u8");
            slave
                .sdo_write(0x1a00, 1, 0x606c_0020u32)
                .await
                .expect("0x1a00 0x606c_0020u32");
            slave
                .sdo_write(0x1a00, 2, 0x6041_0010u32)
                .await
                .expect("0x1a00 0x6041_0010u32");
            // slave.sdo_write(0x1a00, 3, 0x2002_0110u32).await.expect("0x1a00 0x2002_0110u32");
            slave.sdo_write(0x1a00, 0, 2u8).await.expect("0x1a00 2u8");

            slave
                .sdo_write(0x1c12, 0, 0u8)
                .await
                .expect("0x1c12, 0, 0u8");
            slave
                .sdo_write(0x1c12, 1, 0x1600u16)
                .await
                .expect("0x1c12, 1, 0x1600u16");
            slave
                .sdo_write(0x1c12, 0, 0x01u8)
                .await
                .expect("0x1c12, 0, 0x01u8");

            slave
                .sdo_write(0x1c13, 0, 0u8)
                .await
                .expect("0x1c13, 0, 0u8");
            slave
                .sdo_write(0x1c13, 1, 0x1a00u16)
                .await
                .expect("0x1c13, 1, 0x1a01u16");
            slave
                .sdo_write(0x1c13, 0, 0x01u8)
                .await
                .expect("0x1c13, 0, 0x01u8");

            // Opmode - Cyclic Synchronous Velocity
            slave.sdo_write(0x6060, 0, 0x09u8).await.expect("CSV");

            // dbg!(slave.sdo_read::<u8>(0x1c12, 0).await.map(|v| println!("{:x}", v)));
            // dbg!(slave.sdo_read::<u16>(0x1c12, 1).await.map(|v| println!("{:x}", v)));
            // dbg!(slave.sdo_read::<u8>(0x1c13, 0).await.map(|v| println!("{:x}", v)));
            // dbg!(slave.sdo_read::<u16>(0x1c13, 1).await.map(|v| println!("{:x}", v)));
    ...
    log::info!("Moving to DS402 state machine");

    let mut group = group
        .into_pre_op_pdi(&client)
        .await
        .expect("PRE-OP -> PRE-OP (PDI)");
